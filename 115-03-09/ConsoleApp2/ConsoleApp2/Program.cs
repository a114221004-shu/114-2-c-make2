    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            for (outer = 0; outer < row; outer++)
            {
                ...
                WriteLine();
                sum[0] += score[outer, 0];//第 1 欄分數相加
                sum[1] += score[outer, 1];//第 2 欄分數相加
                sum[2] += score[outer, 2];//第 3 欄分數相加
            }
            WriteLine("------------------------");
